...
var creative = (function(window, document, undefined) {
    init = function() {
        settings.debug && console.log('creative.init');

        // define variables
        settings.bannerWidth = document.getElementById("banner").offsetWidth;
        settings.bannerHeight = document.getElementById("banner").offsetHeight;
        creative.complete = false;
        creative.loop = 3;
        ...
        // add eventlisteners
        document.getElementById("banner").addEventListener('mouseenter',onHover)
        document.getElementById("banner").addEventListener('mouseleave',onLeave)
    ...
    play = function() {
        settings.debug && console.log('creative.play')
        gsap.set(".card", {transformStyle:"preserve-3d", perspective:800})
        ...
        .from('.card__wrap', {duration: settings.duration, delay: .5, x: settings.x, scale: settings.scale, y: settings.yPercent, ease: "back.inOut(.75, 1)"}, 'frame1')
        .from('.card', {duration: settings.duration, delay: .5, rotationY: settings.rotationY, rotationZ: settings.rotationZ, rotationX: settings.rotationX, perspective: settings.perspective, ease: "sine.out", onStart: function(){gsap.fromTo('.card', {duration: 1, filter: "blur(2px)"}, {filter: "blur(0px)"})}}, 'frame1')
        .from(frame1b.words, {duration: .5, stagger:.05, yPercent: 100, autoAlpha: 0, ease: "back.out", delay: settings.frame1bDelay}, '<')

        .to(frame1a.words, {duration: .5, stagger:-.05, xPercent: 100, autoAlpha: 0, ease: "back.inOut"}, '+=.5')
    ...
    onHover = function(){
        if(creative.complete){
            gsap.to('.card', {duration: .25, scale: 1.1, ease: "back.out"})
            gsap.to('.shadow', {duration: .25, autoAlpha: 0, y: 10, ease: "back.out"})
            gsap.to('.card', {duration: .25, scale: 1, ease: "back.out", delay: .25})
            gsap.to('.shadow', {duration: .25, autoAlpha: 1, y: 0, ease: "back.out", delay: .25})
        }
    },
    onLeave = function(){
        if(creative.complete){
